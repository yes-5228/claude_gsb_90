...
func Register(router fiber.Router, db *gorm.DB, segments SegmentGateway, snapshots SnapshotGateway) *Service {
	svc := NewService(NewRepository(db), segments, snapshots)
